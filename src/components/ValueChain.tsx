...
const ConnectorDots = ({ x1, x2, y, color }: { x1: number; x2: number; y: number; color: string }) => (
  <g>
    <line x1={x1 + NODE_R + 4} y1={y} x2={x2 - NODE_R - 4} y2={y}
      stroke={color} strokeWidth={0.5} strokeDasharray="4 4" opacity={0.25} />
    {[0, 1, 2].map((dot) => (
      <circle key={dot} r={2.5} cy={y} fill={color} opacity={0}>
        <animate attributeName="cx" from={x1 + NODE_R + 8} to={x2 - NODE_R - 8} dur="2.4s" begin={`${dot * 0.6}s`} repeatCount="indefinite" />
        <animate attributeName="opacity" values="0;0.6;0.6;0" dur="2.4s" begin={`${dot * 0.6}s`} repeatCount="indefinite" />
      </circle>
    ))}
  </g>
);
...
const ValueChain = () => {
  ...
  return (
    <section ref={ref} className="py-20" style={{ backgroundColor: "hsl(220 40% 4%)" }}>
      <div className="absolute left-1/2 -translate-x-1/2 w-[600px] h-[300px] pointer-events-none"
        style={{ background: "radial-gradient(ellipse, rgba(34,211,238,0.05) 0%, transparent 70%)", top: "50%", transform: "translate(-50%, -50%)" }} />

      <div className="max-w-[1200px] mx-auto px-5 relative">
        <div className={`text-center mb-[12px] ${isVisible ? "animate-fade-up" : "opacity-0"}`}>
        ...
        <div className="flex justify-center">
          <div style={{ minWidth: SVG_W }}>
            <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} width="100%" height="auto" className="block">
              {stages.slice(0, -1).map((s, i) => (
                <ConnectorDots key={i} x1={nodeX(i)} x2={nodeX(i + 1)} y={CY} color={s.color} />
              ))}
              {stages.map((s, i) => {
                const cx = nodeX(i);
                return (
                  <g key={i} style={{ color: s.color, opacity: isVisible ? 1 : 0, transform: isVisible ? "translateY(0)" : "translateY(10px)",
                    transition: `opacity 0.5s ease ${i * 0.1}s, transform 0.5s ease ${i * 0.1}s` }}>
                    <circle cx={cx} cy={CY} r={NODE_R + 8} fill="none" stroke={s.color} strokeWidth={0.3} opacity={0.12} />
                    <circle cx={cx} cy={CY} r={NODE_R} fill={s.color} fillOpacity={0.06} stroke={s.color} strokeWidth={0.5} opacity={0.8} />
            ...
            <div className="grid grid-cols-5" style={{ paddingTop: 16 }}>
              {stages.map((s, i) => (
                <div key={i} className="text-center"
                  ...
                  <p className="font-body font-medium whitespace-pre-line" style={{ fontSize: 12.5, color: "rgba(255,255,255,0.75)", lineHeight: 1.3 }}>{s.label}</p>
                  <p className="font-mono font-semibold mt-1.5" style={{ fontSize: 15, color: s.color }}>{s.stat}</p>
                  <p className="font-body" style={{ fontSize: 10, color: "rgba(255,255,255,0.30)" }}>{s.sub}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <p className={`text-center font-body text-muted-foreground max-w-2xl mx-auto mt-10 leading-relaxed ${isVisible ? "animate-fade-up" : "opacity-0"}`}
          style={{ animationDelay: "0.4s", fontSize: "clamp(14px, 2vw, 16px)" }}>
          Other platforms cover pieces of the puzzle. CarbonMap connects them all — the only CCS intelligence tool purpose-built to track the complete value chain from smokestack to reservoir.
        </p>
      </div>
    </section>
  );
